...
describe('Register page', () => {
  test('renders form with inputs and button', () => {
    ...
    expect(screen.getByLabelText(/^Confium password:$/i)).toBeInTheDocument();
  ...
  test('sending data after register', async () => {
    render(<Register />);

    const emailInput = screen.getByLabelText(/e-mail/i);
    const passwordInput = screen.getByLabelText(/^Password:$/i);
    const confPasswordInput = screen.getByLabelText(/^Confium password:$/i);
    const button = screen.getByRole('button', { name: /submit/i });
    ...
    await user.clear(confPasswordInput);
    await user.type(confPasswordInput, 'A4385d11!qwe');
